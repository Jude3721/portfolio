...
function InjuryRow({ player, teamColor, isLast }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: '11px', fontWeight: 700, padding: '3px 8px', borderRadius: '99px', background: s.bg, color: s.color, border: `1px solid ${s.border}`, whiteSpace: 'nowrap', flexShrink: 0, marginTop: '2px' }}>
        {s.label}
      </span>
      <div style={{ flex: 1, minWidth: 0, textAlign: 'left' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
          <span style={{ fontSize: '15px', fontWeight: 700, color: 'rgba(var(--fg-rgb), 0.85)' }}>{player.name}</span>
          {player.pos && (
            <span style={{ fontSize: '11px', fontWeight: 600, padding: '2px 6px', borderRadius: '4px', background: 'rgba(var(--fg-rgb),0.06)', color: 'rgba(var(--fg-rgb),0.4)' }}>{player.pos}</span>
          )}
        </div>
        {player.injuryType && <p style={{ fontSize: '13px', color: 'rgba(var(--fg-rgb),0.5)', marginBottom: '6px' }}>{player.injuryType}</p>}
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
          {player.since && <span style={{ fontSize: '12px', color: 'rgba(var(--fg-rgb),0.28)' }}>등록: {player.since}</span>}
          {player.eta   && <span style={{ fontSize: '12px', fontWeight: 600, color: s.color }}>{player.eta}</span>}
        </div>
      </div>
    </div>
  )
  ...
  return (
    <section style={{ width: '100%', padding: '28px 24px 40px' }}>
      <div style={{ marginBottom: '20px', textAlign: 'left', display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
        <div>
          <h2 style={{ fontSize: '20px', fontWeight: 800, color: 'rgba(var(--fg-rgb),0.88)', letterSpacing: '-0.3px', marginBottom: '4px' }}>구단별 부상 리포트</h2>
          <p style={{ fontSize: '13px', color: 'rgba(var(--fg-rgb),0.32)' }}>구단을 선택하면 현재 부상 · 결장 선수 현황을 확인할 수 있습니다</p>
        </div>
      ...
      <div style={{ borderRadius: '20px', overflow: 'hidden', background: 'var(--neu-bg)', boxShadow: N.raised }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '16px 22px', boxShadow: 'inset 0 -1px 0 rgba(var(--fg-rgb),0.05)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontSize: '15px', fontWeight: 700, color: 'rgba(var(--fg-rgb),0.85)' }}>{team.name} 부상 리포트</span>
          </div>
          <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
            {loading && <span style={{ fontSize: '12px', color: 'rgba(var(--fg-rgb),0.3)' }}>조회 중...</span>}
            {!loading && dlCount > 0    && <span style={{ fontSize: '12px', padding: '3px 10px', borderRadius: '99px', fontWeight: 700, background: 'rgba(248,113,113,0.12)', color: '#f87171', border: '1px solid rgba(248,113,113,0.3)' }}>부상자명단 {dlCount}명</span>}
            {!loading && rehabCount > 0 && <span style={{ fontSize: '12px', padding: '3px 10px', borderRadius: '99px', fontWeight: 700, background: 'rgba(251,146,60,0.12)',  color: '#fb923c', border: '1px solid rgba(251,146,60,0.3)'  }}>재활 {rehabCount}명</span>}
            {!loading && outCount > 0     && <span style={{ fontSize: '12px', padding: '3px 10px', borderRadius: '99px', fontWeight: 700, background: 'rgba(250,204,21,0.12)',  color: '#facc15', border: '1px solid rgba(250,204,21,0.25)'  }}>결장 {outCount}명</span>}
            {!loading && dismissCount > 0 && <span style={{ fontSize: '12px', padding: '3px 10px', borderRadius: '99px', fontWeight: 700, background: 'rgba(148,163,184,0.1)', color: '#94a3b8', border: '1px solid rgba(148,163,184,0.3)' }}>임의해지 {dismissCount}명</span>}
          </div>
        </div>
        ...
        {loading && (
          <div style={{ padding: '56px 0', textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', color: 'rgba(var(--fg-rgb),0.3)' }}>
            ...
            <span style={{ fontSize: '14px' }}>부상자 명단 조회 중...</span>
          </div>
        )}
        ...
        {!loading && players.length === 0 && (
          <div style={{ padding: '56px 0', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: '15px', fontWeight: 600, color: 'rgba(var(--fg-rgb),0.65)', marginBottom: '4px' }}>부상 · 결장 선수 없음</p>
